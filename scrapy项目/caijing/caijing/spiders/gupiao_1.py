...
class Gupiao1Spider(scrapy.Spider):
    ...
    def parse(self, response):
        # print(response.xpath("//div[@id='rzrq']/table[@class='m-table']/tbody/tr/td[2]/a/@href").extract())
        ...
        for a in a_list:  
            ...
            # break   # break表示只用迭代一次即可，不用反复迭代；这样做会只下载一张表中的内容
    ...
    def download_data(self, response): 
        ...
        for tr in tr_list:
            ...
            print( '|+|'.join(td_list) )  # 将td_list列表中的每一个字符串以'|+|'这一个符号进行分割
